...
Namespace Finance_Payment
    Public Class ClsIncomingPayments
                    ...
                    Select Case pVal.EventType
                        ...
                        Case SAPbouiCOM.BoEventTypes.et_COMBO_SELECT
                            If pVal.ColUID = "U_Source" Then
                                Dim ColItem As SAPbouiCOM.Column = objmatrix.Columns.Item("U_AcctCode")
                                ' Dim oLink As SAPbouiCOM.LinkedButton = ColItem.ExtendedObject
                        ...
                        Case SAPbouiCOM.BoEventTypes.et_ITEM_PRESSED
                            ...
                            ElseIf (pVal.ItemUID = "56" Or pVal.ItemUID = "57" Or pVal.ItemUID = "1") And pVal.ActionSuccess Then
                    ...
                    Select Case BusinessObjectInfo.EventType
                        Case SAPbouiCOM.BoEventTypes.et_FORM_DATA_ADD
                            ...
                            If objform.Mode = SAPbouiCOM.BoFormMode.fm_ADD_MODE And BusinessObjectInfo.ActionSuccess = True And BusinessObjectInfo.BeforeAction = False Then
                                DocEntry = objform.DataSources.DBDataSources.Item("ORCT").GetValue("DocEntry", 0)
                                If objform.Items.Item("tjeno").Specific.String <> "" Then
                                    objRs.DoQuery("update OJDT set ""U_TransId""='" & objform.Items.Item("tjeno").Specific.String & "' where ""TransId""=(select ""TransId"" from ORCT where ""DocEntry""='" & DocEntry & "')")
                                    objRs.DoQuery("update OJDT set ""U_IEntry""='" & DocEntry & "' where ""TransId""='" & objform.Items.Item("tjeno").Specific.String & "'")
                                    DocEntry = objform.DataSources.DBDataSources.Item("ORCT").GetValue("TransId", 0)
        ...
        Public Sub CreateButton(ByVal FormUID As String)
            Try
                ...
                If ItemExists(FormUID, "tjeno") = False Then
                    ...
                    objedit.DataBind.SetBound(True, "ORCT", "U_JENo")
                ...
                If ItemExists(FormUID, "chkactive") = False Then
                    ...
                    objItem.Height = objaddon.objapplication.FontHeight + 6 '15
                    objCheck = objItem.Specific
                    objCheck.Caption = "Multi Branch Selection"
                    objCheck.DataBind.SetBound(True, "ORCT", "U_Select")
                End If
                ...
                'objmatrix.Columns.Item("U_AcctName").ChooseFromListUID = "CFL1"
                'objmatrix.Columns.Item("U_AcctName").ChooseFromListAlias = "AcctName"
            ...
            Try
                ...
                Try
                    ...
                    If objform.Items.Item("tjeno").Specific.String = "" Then
                        ...
                        objjournalentry.Reference = "In AccountPay JE"
                        objjournalentry.Reference2 = "In AccPay On: " & Now.ToString
                        ...
                        For AccRow As Integer = 1 To objmatrix.VisualRowCount
                            'Dim vv As String = objmatrix.Columns.Item("5").Cells.Item(AccRow).Specific.String.ToString.Remove(0, 4)
                            'vv = vv.Remove(0, 4)
        ...
        Public Function Cancelling_JournalEntry(ByVal FormUID As String, ByVal JETransId As String) As Boolean
            Try
                Dim TransId As String
                Dim objjournalentry As SAPbobsCOM.JournalEntries
                If JETransId = "" Then Return True
                Try
                    objform = objaddon.objapplication.Forms.Item(FormUID)
                    objmatrix = objform.Items.Item("71").Specific
                    ...
                    strSQL += vbCrLf + "from OJDT T0 join JDT1 T1 ON T0.""TransId""=T1.""TransId"" where  T1.""TransId""='" & JETransId & "'"
                    objRs.DoQuery(strSQL)
                    If objRs.RecordCount = 0 Then Return True
                    If JETransId <> "" Then
                        Dim GetStatus As String = objaddon.objglobalmethods.getSingleValue("select distinct 1 as ""Status"" from OJDT where ""StornoToTr""=" & JETransId & "")
                        If GetStatus = "1" Then Return True
                        objjournalentry = objaddon.objcompany.GetBusinessObject(SAPbobsCOM.BoObjectTypes.oJournalEntries)
                        ...
                        For AccRow As Integer = 0 To objRs.RecordCount - 1
                            strSQL = objaddon.objglobalmethods.getSingleValue("Select 1 as ""Status"" from OACT Where ""AcctCode""='" & objRs.Fields.Item("Account").Value.ToString & "'")
                            If strSQL = "1" Then objjournalentry.Lines.AccountCode = objRs.Fields.Item("Account").Value.ToString Else objjournalentry.Lines.ShortName = objRs.Fields.Item("Account").Value.ToString
                        ...
                        Else
                            ...
                            objaddon.objapplication.StatusBar.SetText("Journal Entry Reversed Successfully...", SAPbouiCOM.BoMessageTime.bmt_Short, SAPbouiCOM.BoStatusBarMessageType.smt_Warning)
                            Return True
                        End If
                    Else
                        Return True
                ...
                Catch ex As Exception
                    If objaddon.objcompany.InTransaction Then objaddon.objcompany.EndTransaction(SAPbobsCOM.BoWfTransOpt.wf_RollBack)
                    objaddon.objapplication.SetStatusBarMessage("JE Reverse Error" & objaddon.objcompany.GetLastErrorDescription, SAPbouiCOM.BoMessageTime.bmt_Medium, True)
                    Return False
                End Try

            Catch ex As Exception
                objaddon.objapplication.SetStatusBarMessage("JE Reverse " & ex.Message, SAPbouiCOM.BoMessageTime.bmt_Medium, True)
                Return False
            Finally
                objRs = Nothing
                GC.Collect()
